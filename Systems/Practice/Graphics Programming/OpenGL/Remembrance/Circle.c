...
int main()
{
    ...
    int capVerts = SEGMENTS + 2,
        sideVerts = SEGMENTS * 6,
        vertexCount = capVerts * 2 + sideVerts;
    float *vertices = malloc(vertexCount * 6 * sizeof(float));

    int i = 0;
    vertices[i++] = 0.0f;
    vertices[i++] = 0.0f;
    vertices[i++] = -HALF_DEPTH;
    vertices[i++] = 0.0f;
    vertices[i++] = 0.0f;
    vertices[i++] = 1.0f;
    ...
    {
        float angle = (2.0f * PI / SEGMENTS) * s;
        vertices[i++] = cos(angle) * RADIUS;
        vertices[i++] = sin(angle) * RADIUS;
        vertices[i++] = HALF_DEPTH;
        vertices[i++] = 0.0f;
        vertices[i++] = 1.0f;
        vertices[i++] = 0.0f;
    ...
    for (int s = 0; s <= SEGMENTS; s++)
    {
        float angle = (2.0f * PI / SEGMENTS) * s; // negative = reversed winding // none reversed to close the gap in width
        vertices[i++] = cos(angle) * RADIUS;
        vertices[i++] = sin(angle) * RADIUS;
        vertices[i++] = -HALF_DEPTH;
        vertices[i++] = 0.0f;
    ...
    // --- SIDE WALL (quads as triangle pairs connecting front to back ring) ---
    for (int s = 0; s < SEGMENTS; s++)
    {
        float a1 = (2.0f * PI / SEGMENTS) * s;
        float a2 = (2.0f * PI / SEGMENTS) * (s + 1);

        float x1 = cos(a1) * RADIUS, y1 = sin(a1) * RADIUS;
        float x2 = cos(a2) * RADIUS, y2 = sin(a2) * RADIUS;
    ...
    char vertexFilePath[] = "Remembrance/Shaders/CoordVert.glsl",
         fragmentFilePath[] = "Remembrance/Shaders/CoordFrag.glsl";
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // side wall
        glDrawArrays(GL_TRIANGLES, capVerts * 2, sideVerts);
