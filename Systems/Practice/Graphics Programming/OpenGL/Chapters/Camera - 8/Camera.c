...
int main(void)
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        vec3 cameraPosition = {camX, 0.0f, camZ},
             cameraTarget = {0.0f, 0.0f, 0.0f},
             worldUp = {0.0f, 1.0f, 0.0f};

        mat4 viewMatrix = GLM_MAT4_IDENTITY_INIT;
        glm_lookat(cameraPosition, cameraTarget, worldUp, viewMatrix);

        mat4 projection = GLM_MAT4_IDENTITY_INIT;
        glm_perspective(glm_rad(45.0f), (float)fbWidth / (float)fbHeight, 0.1f, 100.0f, projection);
