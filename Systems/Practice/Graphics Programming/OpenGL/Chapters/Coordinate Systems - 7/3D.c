...
int main(void)
{
    ...
    unsigned char *data = stbi_load("Chapters/Coordinate Systems - 7/Shaders/Donut-3.png", &width, &height, &nrChannels, 0),
                  *data2 = stbi_load("Chapters/Coordinate Systems - 7/Shaders/graffiti.png", &width2, &height2, &nrChannels2, 0);
    ...
    char vertexFilePath[] = "Chapters/Coordinate Systems - 7/Shaders/3DVert.glsl",
         fragmentFilePath[] = "Chapters/Coordinate Systems - 7/Shaders/3DFrag.glsl";
    ...
    vec3 modelTransform = {1.0f, 0.0f, 0.0f};
    glm_rotate(model, glm_rad(-55.0f), modelTransform);

    // View matrix - view space
    mat4 view = GLM_MAT4_IDENTITY_INIT;
    vec3 viewTransform = {0.0f, 0.0f, -3.0f};
    glm_translate(view, viewTransform);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        int modelLoc = glGetUniformLocation(shaders->shaderProgram, "model");
        glUniformMatrix4fv(modelLoc, 1, GL_FALSE, (mat4 *)model);

        int viewLoc = glGetUniformLocation(shaders->shaderProgram, "view");
        glUniformMatrix4fv(viewLoc, 1, GL_FALSE, (float *)view);

        int projLoc = glGetUniformLocation(shaders->shaderProgram, "projection");
